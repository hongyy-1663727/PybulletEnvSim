...
class JacoEnv(object):
  def __init__(self, mode,angle, dl):
    ...
    # -----------------------
    # For example, position a camera at the side looking at the arm
    self.depth_cam_eye = [-0.35, 0.0, 0.3]     # Camera eye position
    self.depth_cam_target = [-0.35, 0.3, 0.1]  # Camera look-at target
    self.depth_cam_up = [0,0,1]                # Up vector
    self.depth_cam_fov = 60
    self.depth_cam_aspect = 1.0
    self.depth_cam_near = 0.1
    self.depth_cam_far = 2.0
    self.depth_cam_width = 320
    self.depth_cam_height = 240
    
  def get_depth_image(self):
    """Captures a depth image from the predefined camera viewpoint."""
    view_matrix = p.computeViewMatrix(
      cameraEyePosition=self.depth_cam_eye,
      cameraTargetPosition=self.depth_cam_target,
      cameraUpVector=self.depth_cam_up
    )
    projection_matrix = p.computeProjectionMatrixFOV(
      fov=self.depth_cam_fov,
      aspect=self.depth_cam_aspect,
      nearVal=self.depth_cam_near,
      farVal=self.depth_cam_far
    )

    img_arr = p.getCameraImage(
      self.depth_cam_width,
      self.depth_cam_height,
      view_matrix,
      projection_matrix,
      renderer=p.ER_BULLET_HARDWARE_OPENGL
    )
    # img_arr: (width, height, rgbaData, depthData, segmentationData)
    # depthData is a 2D array of floats with values from [0..1]
    # To get the actual depth in meters: depth = depthBuffer*(far-near)+near
    depth_buffer = img_arr[3]
    depth_buffer = np.array(depth_buffer)
    # Convert depth_buffer to actual distance if needed
    # depth_in_meters = depth_buffer*(self.depth_cam_far-self.depth_cam_near)+self.depth_cam_near
    return depth_buffer
